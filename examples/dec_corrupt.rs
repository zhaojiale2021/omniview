...
fn main() {
    let path = std::env::args().nth(1).expect("usage: dec_corrupt <file> <pos>");
    let pos: f64 = std::env::args().nth(2).map(|s| s.parse().unwrap()).unwrap_or(30.0);
    ffmpeg::init().unwrap();
    ...
    let rc = unsafe {
        ffmpeg::ffi::av_seek_frame(input.as_mut_ptr(), stream_index as i32, ts, ffmpeg::ffi::AVSEEK_FLAG_BACKWARD)
    };
    println!("seek rc={rc}");

    let mut scaler = software::scaling::Context::get(
        decoder.format(), decoder.width(), decoder.height(),
        format::Pixel::NV12, decoder.width(), decoder.height(),
        ...
        while decoder.receive_frame(&mut decoded).is_ok() {
            ...
            match pts {
                Some(p) => {
                    ...
                    if first_after_seek {
                        println!("first frame after seek: pts={:.3}s corrupt={} key={}", pts_secs, flags.contains(ffmpeg::util::frame::flag::Flags::CORRUPT), decoded.is_key());
                        first_after_seek = false;
                    }
                    if count < 30 && pts_secs < pos + 0.6 {
                        println!("  frame {count}: pts={pts_secs:.3}s corrupt={} key={}", flags.contains(ffmpeg::util::frame::flag::Flags::CORRUPT), decoded.is_key());
                    }
                }
                None => missing_pts += 1,
            }
    ...
    println!("frames={count} corrupt={corrupt} missing_pts={missing_pts} out_of_order={pts_out_of_order} time={:.1}s", t0.elapsed().as_secs_f64());
}